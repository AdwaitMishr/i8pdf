...
```bash
.venv/bin/python -m factlayer ingest data/starter
.venv/bin/python -m factlayer ingest data/probe/probe-note-delhivery.pdf --collection delhivery
.venv/bin/python -m factlayer ingest data/probe/probe-note-india.pdf --collection india-macroeconomy
...
`scripts/make_probe_pdf.py`, and labelled as such on their own first line). They
exist because the six starter documents contain no high-confidence
